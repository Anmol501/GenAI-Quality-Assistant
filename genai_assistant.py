...
Original file is located at
    https://colab.research.google.com/drive/1D0L66A5077EzkujFK1DJ2xW03S4d-zl2
"""
    ...
    for section, data in responses.items():
        if data["skipped"]:
            ...
            for subtopic, answer in data["answers"].items():
                if answer == "y":
                    summary.append(f"  {subtopic} — Followed")
                elif answer == "n":
                    summary.append(f"  {subtopic} — Needs Improvement")
                else:
                    summary.append(f"  {subtopic} — No response")
...
for line in lines:
    if re.match(r"^\d+\.\s", line):
        if current_subtopic:
            recommendation_dict[current_subtopic] = "📌 " + " ".join(current_points)
...
def generate_pdf_report(user_answers, recommendations_dict, filename="MSME_Quality_Report.pdf"):
    ...
    for topic, result in user_answers.items():
        ...
        story.append(Paragraph(f"<b>{topic}</b> — {status}", styles['Heading3']))
